...
with tempfile.TemporaryDirectory(prefix='decklock-release-test-') as temp:
    ...
    for name in ['check-release-tag.py', 'release-notes.py', 'release-channel.py']:
    ...
    for version in ['0.2.0', '0.2.1', '0.2.10', '0.3.0-rc.1']:
        (fixture/'Cargo.toml').write_text(f'[package]\nversion="{version}"\n')
        assert run('release-channel.py').strip() == ('prerelease' if '-' in version else 'release')
        for revision in [1, 2]:
        ...
        assert ('experimental prerelease' in notes) == ('-' in version)
...
print('PASS: exact version/revision tags, release channel, reviewed notes, required release date and missing-section rejection')
