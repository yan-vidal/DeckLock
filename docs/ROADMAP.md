...
- Phase A — more Wayland compositors (#9): verification only, same binary and same package / mais compositores Wayland: só verificação, mesmo binário e mesmo pacote.
  ...
  Sway runs the full real-PAM suite in each distribution VM, and labwc now repeats the lock boundary there too (input isolation, real PAM denial and unlock, input restored, killed locker stays locked). Hyprland, niri, river, Wayfire and COSMIC remain protocol-only: Hyprland and Wayfire need a GPU render node the guest does not have, niri and COSMIC have no headless mode, and river is not packaged on Ubuntu 26.04.
  O Sway roda a bateria completa com PAM real na VM de cada distribuição, e o labwc agora repete lá a fronteira do bloqueio (isolamento de entrada, recusa e desbloqueio pelo PAM real, entrada restaurada, bloqueador encerrado mantém a sessão bloqueada). Hyprland, niri, river, Wayfire e COSMIC seguem só com conformidade de protocolo: Hyprland e Wayfire precisam de um render node de GPU que a VM não tem, niri e COSMIC não têm modo headless e o river não é empacotado no Ubuntu 26.04.
...
- Architectures — aarch64 Fedora and Ubuntu candidate packages are built and tested in CI on arm64 runners (Arch has no official aarch64 image). They stay unpublished until a VM gate runs them; hosted arm64 runners offer no KVM / arquiteturas — pacotes candidatos aarch64 de Fedora e Ubuntu são compilados e testados no CI em runners arm64 (o Arch não tem imagem aarch64 oficial). Seguem sem publicação até um gate de VM executá-los; os runners arm64 hospedados não oferecem KVM.

Still open in this reach work: the Debian package (#25, waiting for gtk4-layer-shell 1.2 in Debian), VM coverage for compositors that need a GPU, and an aarch64 VM gate.
Ainda em aberto nesse trabalho de alcance: o pacote Debian (#25, aguardando gtk4-layer-shell 1.2 no Debian), cobertura em VM para compositores que precisam de GPU e um gate de VM aarch64.
