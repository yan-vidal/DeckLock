...
- Phase C — X11 lock backend behind a compatibility layer (#15-#18); implemented behind the optional `x11` feature, off by default and not built into the published packages, with accelerated video (#18) still open / backend de bloqueio X11 atrás de uma camada de compatibilidade; implementado atrás da feature opcional `x11`, desligada por padrão e fora dos pacotes publicados, com o vídeo acelerado (#18) ainda em aberto.
