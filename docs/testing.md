...
| aarch64 packages | `package-fedora-aarch64` and `package-ubuntu-aarch64` jobs on `ubuntu-24.04-arm` | The same release build, `cli_contract` and `session_invariants` tests and package contract as x86_64. Still unpublished candidates |
| aarch64 VM gate | `vm-ubuntu-aarch64` on `ubuntu-24.04-arm`, `scripts/test-vm.py --arch aarch64 --allow-emulation` | The whole Ubuntu guest suite (Sway with real PAM, X11, labwc, Wayfire, greetd) on the arm64 package, in a TCG-emulated guest with every bounded wait scaled by 8; about 100 minutes. Fedora arm64 has no VM target yet |
