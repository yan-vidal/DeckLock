...
Os avisos de bloqueio vêm exclusivamente das mensagens do PAM. Mensagens reconhecidas de pam_faillock são traduzidas; outras mensagens sanitizadas permanecem como texto simples. O DeckLock não lê faillock.conf nem executa faillock(8): argumentos dos módulos, pilhas de serviços e janelas de falhas podem tornar incorreto um cálculo separado da política. O contador usa um prazo em CLOCK_BOOTTIME, incluindo suspensão; atrasos nos callbacks do GTK apenas atrasam o desenho, sem estender o prazo. Uma nova autenticação cancela o contador anterior. A saída do auxiliar é drenada sem bloqueio, retendo no máximo 512 bytes e respeitando o timeout de autenticação mesmo se processos descendentes herdarem o pipe.
