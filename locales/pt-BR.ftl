...
unsupported = Este desktop não permite que outros programas forneçam a tela de bloqueio. O DeckLock precisa de um compositor com ext-session-lock-v1, como Sway ou Hyprland; GNOME e KDE Plasma não são suportados.
...
no-wayland-sessions = Nenhuma sessão de desktop Wayland está instalada.
login = Entrar
...
auth-locked-in = Conta bloqueada por falhas de autenticação. Espera estimada: %s.
